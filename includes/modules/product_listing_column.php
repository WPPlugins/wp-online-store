...
// optional Product List Filter
  ...
  $prod_list_contents .= '        <td  style="text-align:right;"><strong>';
	...
	$prod_list_contents .=' <a href="'. tep_href_link(basename($_GET['slug']), tep_get_all_get_params(array('view')) . 'view=grid', $request_type).'">Grid View |</a> <a href="'. tep_href_link(basename($_GET['slug']), tep_get_all_get_params(array('view')) . 'view=list', $request_type).'"> List View</a></strong>';
  ...
  if ($listing_split->number_of_rows > 0) {
    ...
    while ($listing = tep_db_fetch_array($listing_query)) {
      ...
      $prod_list_contents .= ' <td><div class="borderdiv1"><table border="0" width="100%" cellspacing="2" cellpadding="2"   >
	  							<tr><td style="text-align:center"><a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $listing['products_id']) . '">' . tep_image(DIR_WS_IMAGES . $listing['products_image'], $listing['products_name'], $w, $h) . '</a></td ></tr>
								<tr><td style="text-align:center"><a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $listing['products_id']) . '">' . $listing['products_name'] . '</a>'. '</td ></tr>
								<tr><td style="text-align:center">' . $currencies->display_price($listing['products_price'], tep_get_tax_rate($listing['products_tax_class_id'])).'<a href="'. tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $listing['products_id']).'"> </td ></tr>
								<tr><td style="text-align:center">'.tep_draw_button(IMAGE_BUTTON_BUY_NOW, 'cart', tep_href_link(basename($PHP_SELF), tep_get_all_get_params(array('action')) . 'action=buy_now&products_id=' . $listing['products_id'])) .'</td ></tr></table></div></td>
							   ';
