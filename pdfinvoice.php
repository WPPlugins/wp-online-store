...
function output_table_heading($Y_Fields_Name_position,$pdf,$cell_color){
...
output_table_heading($Y_Fields_Name_position,$pdf,$cell_color);
...
////	
// He añadido esto para facilitar el cambio del numero de lineas por pagina.
// El valor de "$lines_page" es el que controla el numero de lineas (items) de la primera pagina.
// El valor de "$lines_page_2" es el que controla el numero de lineas (items) de las demas paginas.
// Cambiando estos valores cambiamos toda la configuracion de una sola vez.
$lines_page = PDF_INV_NUM_ITEMS_PAGE_1;
$lines_page_2 = PDF_INV_NUM_ITEMS_PAGE_2;

//// Modificacion para corregir paginacion sin productos.
// Con este bloque evitamos que la segunda pagina se vea solamente con la totalizacion y sin ninguna linea de productos. 
// Quitamos una linea a la primera pagina y se la pasamos a la ultima pagina cuando se produce esta situacion.
// Gracias a Eusebio que me facilito el codigo para hacer la consulta y sacar el numero de lineas del pedido.

if ($items_total == $lines_page){	$linfac = $lines_page;	}else{		// Correcion items Pagina 1
if ($items_total == $lines_page+$lines_page_2){		$linfac = $lines_page-2;	}else{		// Correcion items Pagina 2
if ($items_total == $lines_page+($lines_page_2*2)){		$linfac = $lines_page-2;	}else{		// Correcion items Pagina 3
if ($items_total == $lines_page+($lines_page_2*3)){		$linfac = $lines_page-2;	}else{		// Correcion items Pagina 4
if ($items_total == $lines_page+($lines_page_2*4)){		$linfac = $lines_page-2;	}else{		// Correcion items Pagina 5
if ($items_total == $lines_page+($lines_page_2*5)){		$linfac = $lines_page-2;	}else{		// Correcion items Pagina 6
if ($items_total == $lines_page+($lines_page_2*6)){		$linfac = $lines_page-2;	}else{		// Correcion items Pagina 7
if ($items_total == $lines_page+($lines_page_2*7)){		$linfac = $lines_page-2;	}else{		// Correcion items Pagina 8
if ($items_total == $lines_page+($lines_page_2*8)){		$linfac = $lines_page-2;	}else{		// Correcion items Pagina 9
if ($items_total == $lines_page+($lines_page_2*9)){		$linfac = $lines_page-2;	}else{		// Correcion items Pagina 10
$linfac = $lines_page-1;		} } } } } } } } } }
//// Fin de Modificacion para corregir paginacion sin productos.

    //Check for product line overflow
     $item_count++;
    if ((is_long($item_count / ($lines_page_2)) && $i >= $linfac) || ($i == $linfac)){
        $pdf->AddPage();
        //Fields Name position
        $Y_Fields_Name_position = 125;
        //Table position, under Fields Name
        $Y_Table_Position = 70;
        output_table_heading($Y_Table_Position-6,$pdf,$cell_color);
	if ($i == $linfac) $item_count = 0;
    }
}
for ($i = 0, $n = sizeof($order->totals); $i < $n; $i++) {
    ...
    echo "<a href='".get_bloginfo("url")."/".$file_name."'>Download</a>";
