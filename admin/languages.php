...
*/
  ...
  // we cant use get in form posting
...
// create additional products_options_values records
		...
		//echo "<br>".
		 $check_query = tep_db_query("select * from products_options_values where products_options_values_name = 'TEXT' and language_id = '".$insert_id."'");
        if (tep_db_num_rows($check_query) == 0) {
		
		$sql = "insert into " . TABLE_PRODUCTS_OPTIONS_VALUES . " (products_options_values_id, language_id, products_options_values_name) values ('" . (int)$products_options_values['products_options_values_id'] . "', '" . (int)$insert_id . "', '" . tep_db_input($products_options_values['products_options_values_name']) . "')";
          tep_db_query($sql);
        }
...
// create additional orders_status records
        ...
        if (isset($HTTP_POST_VARS['default']) && ($HTTP_POST_VARS['default'] == 'on')) {
         ...
         tep_redirect(tep_href_link(FILENAME_LANGUAGES, (isset($HTTP_GET_VARS['pages']) ? 'pages=' . $HTTP_GET_VARS['pages'] . '&' : '') . 'lID=' . $insert_id));
      ...
      case 'save':
        ...
        tep_redirect(tep_href_link(FILENAME_LANGUAGES, 'pages=' . $HTTP_GET_VARS['pages'] . '&lID=' . $HTTP_GET_VARS['lID']));
      ...
      case 'deleteconfirm':
        ...
        tep_redirect(tep_href_link(FILENAME_LANGUAGES, 'pages=' . $HTTP_GET_VARS['pages']));
...
<?php
  ...
  $languages_split = new splitPageResults($HTTP_GET_VARS['pages'], MAX_DISPLAY_SEARCH_RESULTS, $languages_query_raw, $languages_query_numrows);
  ...
  while ($languages = tep_db_fetch_array($languages_query)) {
    ...
    if (isset($lInfo) && is_object($lInfo) && ($languages['languages_id'] == $lInfo->languages_id) ) {
      ...
      echo '                  <tr id="defaultSelected" class="dataTableRowSelected" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . tep_href_link(FILENAME_LANGUAGES, 'pages=' . $HTTP_GET_VARS['pages'] . '&lID=' . $lInfo->languages_id . '&action=edit') . '\'">' . "\n";

    } else {

      echo '                  <tr class="dataTableRow" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . tep_href_link(FILENAME_LANGUAGES, 'pages=' . $HTTP_GET_VARS['pages'] . '&lID=' . $languages['languages_id']) . '\'">' . "\n";
...
?>
                ...
                <td class="dataTableContent" align="right"><?php if (isset($lInfo) && is_object($lInfo) && ($languages['languages_id'] == $lInfo->languages_id)) { echo tep_image(DIR_WS_IMAGES . 'icon_arrow_right.gif'); } else { echo '<a href="' . tep_href_link(FILENAME_LANGUAGES, 'pages=' . $HTTP_GET_VARS['pages'] . '&lID=' . $languages['languages_id']) . '">' . tep_image(DIR_WS_IMAGES . 'icon_info.gif', IMAGE_ICON_INFO) . '</a>'; } ?>&nbsp;</td>
              ...
              <tr>
                ...
                <td colspan="3"><table border="0" width="100%" cellspacing="0" cellpadding="2">
                  ...
                  <tr>
                    ...
                    <td class="smallText" valign="top"><?php echo $languages_split->display_count($languages_query_numrows, MAX_DISPLAY_SEARCH_RESULTS, $HTTP_GET_VARS['pages'], TEXT_DISPLAY_NUMBER_OF_LANGUAGES); ?></td>

                    <td class="smallText" align="right"><?php echo $languages_split->display_links($languages_query_numrows, MAX_DISPLAY_SEARCH_RESULTS, MAX_DISPLAY_PAGE_LINKS, $HTTP_GET_VARS['pages'],'','pages'); ?></td>
...
?>
                  ...
                  <tr>
                    ...
                    <td class="smallText" align="right" colspan="2"><?php echo tep_draw_button(IMAGE_NEW_LANGUAGE, 'plus', tep_href_link(FILENAME_LANGUAGES, 'pages=' . $HTTP_GET_VARS['pages'] . '&lID=' . $lInfo->languages_id . '&action=new')); ?></td>
...
<?php
  ...
  switch ($action) {
    ...
    case 'new':
      ...
      $contents[] = array('align' => 'center', 'text' => '<br />' . tep_draw_button(IMAGE_SAVE, 'disk', null, 'primary') . tep_draw_button(IMAGE_CANCEL, 'close', tep_href_link(FILENAME_LANGUAGES, 'pages=' . $HTTP_GET_VARS['pages'] . '&lID=' . $HTTP_GET_VARS['lID'])));
    ...
    case 'edit':
      ...
      $contents = array('form' => tep_draw_form('languages', FILENAME_LANGUAGES, 'pages=' . $HTTP_GET_VARS['pages'] . '&lID=' . $lInfo->languages_id . '&action=save'));
      ...
      $contents[] = array('align' => 'center', 'text' => '<br />' . tep_draw_button(IMAGE_SAVE, 'disk', null, 'primary') . tep_draw_button(IMAGE_CANCEL, 'close', tep_href_link(FILENAME_LANGUAGES, 'pages=' . $HTTP_GET_VARS['pages'] . '&lID=' . $lInfo->languages_id)));
    ...
    case 'delete':
      ...
      $contents[] = array('align' => 'center', 'text' => '<br />' . (($remove_language) ? tep_draw_button(IMAGE_DELETE, 'trash', tep_href_link(FILENAME_LANGUAGES, 'pages=' . $HTTP_GET_VARS['pages'] . '&lID=' . $lInfo->languages_id . '&action=deleteconfirm'), 'primary') : '') . tep_draw_button(IMAGE_CANCEL, 'close', tep_href_link(FILENAME_LANGUAGES, 'pages=' . $HTTP_GET_VARS['pages'] . '&lID=' . $lInfo->languages_id)));
    ...
    default:
      ...
      if (is_object($lInfo)) {
        ...
        $contents[] = array('align' => 'center', 'text' => tep_draw_button(IMAGE_EDIT, 'document', tep_href_link(FILENAME_LANGUAGES, 'pages=' . $HTTP_GET_VARS['pages'] . '&lID=' . $lInfo->languages_id . '&action=edit')) . tep_draw_button(IMAGE_DELETE, 'trash', tep_href_link(FILENAME_LANGUAGES, 'pages=' . $HTTP_GET_VARS['pages'] . '&lID=' . $lInfo->languages_id . '&action=delete')) . tep_draw_button(IMAGE_DETAILS, 'info', tep_href_link(FILENAME_DEFINE_LANGUAGE, 'lngdir=' . $lInfo->directory)));
